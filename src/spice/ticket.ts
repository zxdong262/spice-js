...
import { RSAKey } from './thirdparty/rsa'
import { BigInteger } from './thirdparty/jsbn'
import { SecureRandom } from './thirdparty/rng'
import { rstr_sha1 } from './thirdparty/sha1'
...
function MGF1 (mask: number[], seed: number[]): number {
  ...
  for (i = 0, outlen = 0; outlen < mask.length; i++) {
    ...
    const combo_hash = rstr_sha1(combo_buf)
...
function RSA_padding_add_PKCS1_OAEP (tolen: number, from: string, param: string): string | null {
  const seed = new Array(SHA_DIGEST_LENGTH)
  const rand = new SecureRandom()
  rand.nextBytes(seed)

  const dblen = tolen - 1 - seed.length
  const db = new Array(dblen)
  const padlen = dblen - from.length - 1
  let i: number
  ...
  const param_hash = rstr_sha1(param)
  ...
  const seedmask = Array(SHA_DIGEST_LENGTH)
...
function asn_get_length (u8: Uint8Array, at: number): [number, number] | null {
  let len = u8[at++]
  if (len > 0x80) {
    if (len != 0x81) {
...
function find_sequence (u8: Uint8Array, at?: number): [number, number] | null {
  let lenblock
  at = at || 0
  if (u8[at++] != 0x30) {
...
export function rsa_encrypt (rsa: RSAKey, str: string): number[] | null {
  let i: number
  const ret: number[] = []
  if (!rsa.n) {
    return null
  ...
  const ba = new Array(oaep.length)

  for (i = 0; i < oaep.length; i++) {
    ba[i] = oaep.charCodeAt(i)
  }
  const bigint = new BigInteger(ba)
  const enc = rsa.doPublic(bigint)
  let h = enc.toString(16)
  if ((h.length & 1) != 0) {
    h = '0' + h
  }
