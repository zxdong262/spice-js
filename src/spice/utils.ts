...
import { code_to_scancode } from './code_to_scancode'

/* ----------------------------------------------------------------------------
**  Utility settings and functions for Spice
**-------------------------------------------------------------------------- */
const DEBUG = 0
const PLAYBACK_DEBUG = 0
const STREAM_DEBUG = 0
const DUMP_DRAWS = false
const DUMP_CANVASES = false
...
**-------------------------------------------------------------------------- */
const EMPTY_GIF_IMAGE = 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs='
...
**-------------------------------------------------------------------------- */
function combine_array_buffers (a1: ArrayBuffer, a2: ArrayBuffer): ArrayBuffer {
...
**  hexdump_buffer
**-------------------------------------------------------------------------- */
function hexdump_buffer (a: ArrayBuffer): void {
...
**-------------------------------------------------------------------------- */
function arraybuffer_to_str (buf: ArrayBuffer): string {
...
**-------------------------------------------------------------------------- */
const common_scanmap: { [key: number]: number } = []
...
common_scanmap[44] = KeyNames.KEY_SysRequest
...
const firefox_scanmap: { [key: number]: number } = []
firefox_scanmap[173] = KeyNames.KEY_Minus
firefox_scanmap[61] = KeyNames.KEY_Equal
firefox_scanmap[59] = KeyNames.KEY_SemiColon

/* DOM3 codes */
const DOM_scanmap: { [key: number]: number } = []
DOM_scanmap[189] = KeyNames.KEY_Minus
DOM_scanmap[187] = KeyNames.KEY_Equal
DOM_scanmap[186] = KeyNames.KEY_SemiColon

function get_scancode (keyCode: number, code: string): number | undefined {
  if (code_to_scancode[code] !== undefined) {
    return code_to_scancode[code]
  }

  if (common_scanmap[keyCode] === undefined) {
    if (typeof navigator !== 'undefined' && navigator.userAgent.includes('Firefox')) { return firefox_scanmap[keyCode] } else { return DOM_scanmap[keyCode] }
  } else { return common_scanmap[keyCode] }
}

function keycode_to_start_scan (keyCode: number, code: string): number {
...
function keycode_to_end_scan (keyCode: number, code: string): number {
  const scancode = get_scancode(keyCode, code)
...
function dump_media_element (m: HTMLMediaElement): string {
  const ret = '[networkState ' + m.networkState +
...
function dump_media_source (ms: MediaSource): string {
  const ret = '[duration ' + ms.duration +
            '|readyState ' + ms.readyState + ']'
  return ret
}

function dump_source_buffer (sb: SourceBuffer): string {
  const ret = '[appendWindowStart ' + sb.appendWindowStart +
...
function dump_timerange (tr: TimeRanges | null): string {
  let ret: string
