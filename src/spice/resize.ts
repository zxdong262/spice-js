...
**-------------------------------------------------------------------------- */
function resize_helper (sc: any) {
  let w = document.getElementById(sc.screen_id).clientWidth
  const m = document.getElementById(sc.message_id)
  ...
  if (m != null) {
    if (window.getComputedStyle(m).getPropertyValue('display') == 'none') {
      ...
      const mh = parseInt(window.getComputedStyle(m).getPropertyValue('height'), 10)
      h = h - mh
    } else {
...
function handle_resize (e: Event) {
